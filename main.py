...
def get_all_songs(limit=10, offset=0):
    conn = sqlite3.connect("songs.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, keyword, file_id, title FROM songs ORDER BY id LIMIT ? OFFSET ?", (limit, offset))
    results = []
    for row in cursor.fetchall():
        results.append({"id": row[0], "keyword": row[1], "file_id": row[2], "title": row[3]})
    conn.close()
    return results
...
def get_song_by_id(song_id):
    conn = sqlite3.connect("songs.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, keyword, file_id, title FROM songs WHERE id = ?", (song_id,))
    result = cursor.fetchone()
    conn.close()
    if result:
        return {"id": result[0], "keyword": result[1], "file_id": result[2], "title": result[3]}
    return None

def get_song_by_keyword(keyword):
    conn = sqlite3.connect("songs.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, keyword, file_id, title FROM songs WHERE LOWER(keyword) = ?", (keyword.lower(),))
    result = cursor.fetchone()
    conn.close()
    if result:
        return {"id": result[0], "keyword": result[1], "file_id": result[2], "title": result[3]}
...
def add_song(keyword, file_id, title):
    conn = sqlite3.connect("songs.db")
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO songs (keyword, file_id, title) VALUES (?, ?, ?)",
...
def get_songs_list(page=0):
    ...
    builder.row(InlineKeyboardButton(text=f"📄 Страница {page+1}/{total_pages}", callback_data="noop"))
...
async def list_cmd(message: types.Message):
    total = get_total_songs()
    if total == 0:
        await message.answer("📋 В коллекции пока нет песен!", reply_markup=get_main_menu())
        return
    await message.answer(
        f"📋 Список песен\n\nВсего: {total}",
        reply_markup=get_songs_list(0)
    )
...
@dp.message(lambda msg: msg.audio or (msg.document and msg.document.file_name and msg.document.file_name.endswith('.mp3')))
...
async def process_title(message: types.Message, state: FSMContext):
    ...
    if existing:
        await message.answer(f"⚠️ Песня '{title}' уже есть!", reply_markup=get_main_menu())
        await state.clear()
        return
...
async def callback(call: types.CallbackQuery):
    ...
    if data == "menu_main":
        ...
        total = get_total_songs()
        if total == 0:
            await call.message.edit_text("📋 В коллекции пока нет песен!", reply_markup=get_main_menu())
            return
        await call.message.edit_text(
            f"📋 Список песен\n\nВсего: {total}",
            reply_markup=get_songs_list(0)
        ...
        total = get_total_songs()
        history_text = "\n".join([f"{i+1}. {h}" for i, h in enumerate(search_history[-10:][::-1])]) if search_history else "Пока пусто"
        await call.message.edit_text(
            f"📊 Статистика\n\n🎵 Всего песен: {total}\n\n📝 История поиска:\n{history_text}",
            reply_markup=get_main_menu()
        )
        ...
        await call.message.edit_text(
            f"📋 Список песен\n\nВсего: {total}",
            reply_markup=get_songs_list(page)
        )
